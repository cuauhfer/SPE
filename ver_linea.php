...
<head>
	...
	<!--Titulo-->
	<title>Lineas de Innovación</title>
</head>
...
<body>
	...
	<br><br><br><br>
			<!--Vista rapida-->
			<h1 align="center">Lineas de innovación</h1>
			<br>
			<?php  
				...
				if(isset($_POST['buscar'])){
					...
					$sql = "SELECT * FROM lineainn WHERE `nombre` LIKE '%$bus%' ORDER BY `lineainn`.`id`";
					$resultado = mysqli_query($conexion, $sql);

					while($reg = mysqli_fetch_array($resultadoautor)){
						$codigo = $reg['codigo'];
						$sql = "SELECT * FROM lineainn WHERE `codigoPersona` LIKE '%$codigo%' OR `nombre` LIKE '%$bus%' ORDER BY `lineainn`.`id`";
						$resultado = mysqli_query($conexion, $sql);
					}
				}
				else{
